...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            #To include an action in the graph, The action should statisfy previous state level.
            if action_node.prenodes.issubset(self.s_levels[level]):
                
            #Add the actions and also add it the state and action nodes    
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #We need to check all the possibilites if there is an action which
        #conflicts with the precondition negative and vice - versa
        for effect in node_a1.action.effect_add:
